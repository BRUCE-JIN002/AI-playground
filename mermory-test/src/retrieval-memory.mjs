 ...
 * 从 Milvus 中检索相关的历史对话
 */
async function retrieveRevelantConversations(query, k = 2) {
  try {
    // 生成查询的向量
...
async function retrievalMemoryDemo() {
  ...
  for (let i = 0; i < conversations.length; i++) {
    ...
    console.log("\n【检索相关历史对话】");
    const retrievedConversations = await retrieveRevelantConversations(
      input,
      2,
    );
    let revelantHistory = "";
    if (retrievedConversations.length > 0) {
      ...
      revelantHistory = retrievedConversations
        .map((conv, idx) => {
          return `[历史对话 ${idx + 1}]
                轮次: ${conv.round}
                ${conv.content}`;
    ...
    const contextMessages = revelantHistory
      ? [
          new HumanMessage(
            `相关历史对话：\n${revelantHistory}\n\n用户问题: ${input}`,
          ),
